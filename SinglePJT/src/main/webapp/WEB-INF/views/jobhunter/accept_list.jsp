...
<div class="d-flex align-items-center p-3 my-3 text-white bg-purple rounded shadow-sm">
   ...
   <div class="row">
   		<h3 class="border-bottom pb-2 mb-0 col-12">지원한 구인공고</h3>
   	</div>
   	<div class="row">
     <table>
        ...
        <tr>
           ...
           <c:choose>
	           <c:when test="${sessionScope.user_check eq '1' }">
		           <td class="col-1 border-bottom pt-3" align="center">
		              <a href="javascript:;" onclick="deleteAccept(${list.jobsearch_num})" style="">취소하기</a>
		           </td>
	           </c:when>
...
function deleteAccept(jobsearch_num) {
	
	if(confirm("해당 공고에 지원 내용을 취소하시겠습니까?")){
		$.ajax({
			url:"/jobhunter/delete",
			data:{"jobsearch_num":jobsearch_num},
			datatype:"json",
			type:"post",
			success : function(result) {
				console.log(result)
				if(result=="1"){
					alert("공고 지원이 취소되었습니다.");
					location.reload();
				}
			}
		})
